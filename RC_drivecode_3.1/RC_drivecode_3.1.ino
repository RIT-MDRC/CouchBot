...
#define MIN_SPEED_MULTIPLIER 0.2  // never 0 else no motion when LY down all the way
#define MAX_SPEED_MULTIPLIER 2
#define MIN_TURN_MULTIPLIER 0.5  // never 0 else no motion when LY down all the way
#define MAX_TURN_MULTIPLIER 0.6
#define MAX_ACCEL 20
...
void update_joystick_to_tank_setpoints(void) {
  ...
  // Set multipliers from left Y
  turn_multiplier = float(map(ly, LY_UP, LY_DOWN, MIN_TURN_MULTIPLIER*128, MAX_TURN_MULTIPLIER*128))/128; 
  speed_multiplier = float(map(ly, LY_UP, LY_DOWN, MIN_SPEED_MULTIPLIER*128, MAX_SPEED_MULTIPLIER*128))/128; 
  Serial.print(speed_multiplier);
  Serial.print(",");
  Serial.println(turn_multiplier);
  
  // 1&2. scale inputs and invert X
  int X = map(rx, RX_LEFT, RX_RIGHT, -100 * turn_multiplier, 100 * turn_multiplier);
  int Y = map(ry, RY_DOWN, RY_UP,    -100 * speed_multiplier, 100 * speed_multiplier);
